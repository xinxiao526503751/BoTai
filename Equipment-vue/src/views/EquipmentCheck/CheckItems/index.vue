  ...
  <div class="flex-column">
    ...
    >
      <el-form :rules="rules" ref="dataForm" :model="temp" label-width="110px">
        <el-form-item label="点检项名称：" prop="CHECK_ITEM_NAME" size="small">
          <el-input v-model="temp.CHECK_ITEM_NAME"></el-input>
        </el-form-item>
        <el-form-item
          :label="'点检项类型：'"
          size="small"
        ...
        <!-- <el-form-item
          :label="'点检项类型：'"
          size="small"
          prop="examitem_type_name"
          v-show="dialogStatus == 'update'"
        >
          <el-input v-model="temp.examitem_type_name" :disabled="true">
...
export default {
  ...
  created() {
    this.getEquTypePage()
    this.getCheckItemsPageList()
    this.getTree()
  },
  methods: {
    // 获取所有点检项目函数
    getAllCheckItem() {
      CheckItem.GetAllCheckItem().then((response) => {
        ...
        //this.tempCheckItems = JSON.parse(JSON.stringify(tempData))
      })
    },
    // 获取所有设备类型函数
    getAllEquType() {
      ...
      EquInfo.GetAllEquType().then((response) => {
        ...
        //this.tempEquTypes=JSON.parse(JSON.stringify(tempData))
        //console.log('-----临时数据',this.tempEquTypes)
    ...
    async getTree() {
      ...
      await CheckItem.GetAllCheckItem().then((response) => {
        tempCheckItem = response.Data.map(function (item, index, input) {
          return {
            id: item.CHECK_ITEM_ID,
            label: item.CHECK_ITEM_NAME,
            parentId: item.TYPE_ID
      ...
      var treeData = TwoListToTree(tempEquType, tempCheckItem)
      this.equipmentTree = treeData
      this.listLoading = false
      //console.log('------树型数据',treeData)
    },
    // 点检项分页数据获取
    getCheckItemsPageList(){
      this.listLoading = true
      CheckItem.QueryAndPage(this.listQuery).then((response)=>{
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.addCheckItem(this.temp)
          this.getTree()
        }
      })
    },
    //新增接口函数
    addCheckItem(addData) {
      CheckItem.AddCheckItem(addData).then((response) => {
        if (response.Code === 200) {
    ...
    queryAndPage(data){
      this.listLoading=true
      CheckItem.QueryAndPage(data).then((response)=>{
        if(response.Code===200){
    ...
    onDelete() {
      ...
      for (let i = 0; i < array.length; i++) {
        ids.push(array[i].CHECK_ITEM_ID)
      }
    ...
    // 可批量删除函数
    deleteCheckItemByIds(ids) {
      CheckItem.DeleteByIds(ids).then((response) => {
        if (response.Code === 200) {
          this.$notify({
            title: '成功',
            message: '删除成功',
    ...
    handleNodeClick(data) {
      ...
      if (data.children !== null) {
        this.getCheckItemsByTypeId(this.listQuery)
      }
    },
    // 节点点击函数
