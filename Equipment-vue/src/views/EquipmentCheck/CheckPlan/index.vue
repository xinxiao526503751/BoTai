    ...
    <div class="app-container flex-item">
      <div class="bg-white" style="height: 100%">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <span>{{ scope.row.CHECK_METHOD }}</span>
            </template>
          </el-table-column>
          <el-table-column
            align="center"
          ...
          >
            <template slot-scope="scope">
              <span>{{ scope.row.CHECK_FREQUENCY }}</span>
            </template>
          </el-table-column>
          <el-table-column
            align="center"
            ...
            <el-row>
              <el-col :span="8">
                <el-form-item
                  ...
                  prop="CHECK_PERSON"
                >
                  <el-input v-model="temp.CHECK_PERSON"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item
                  ...
                  prop="CHECK_METHOD"
                >
                  <el-input v-model="temp.CHECK_METHOD"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item
...
export default {
  ...
  methods: {
    ...
    // 点检计划分页数据接口函数
    getCheckPlanPageList(){
      this.listLoading = true
      CheckPlan.QueryCheckPlan(this.listQuery).then((response)=>{
    ...
    // 模糊查询接口函数
    queryCheckPlan(params) {
      CheckPlan.QueryCheckPlan(params).then((response) => {
        if (response.Code === 200) {
          this.tableTotalData = response.Data
    ...
    queryAndPage(data){
      this.listLoading = true
      CheckPlan.QueryCheckPlan(data).then((response)=>{
        if(response.Code===200){
          this.searchStatus=true
          this.list = response.Data
          this.total = response.Total
    ...
    // 新增接口函数
    addCheckPlan(addData) {
      CheckPlan.AddCheckPlan(addData).then((response) => {
        if (response.Code === 200) {
    ...
    // 批量删除接口函数
    deleteCheckPlanByIds(ids) {
      CheckPlan.DeleteByIds(ids).then((response) => {
        if (response.Code === 200) {
          this.$notify({
            title: '成功',
            message: '删除成功',
    ...
    save() {
      ...
      this.temp.CHECK_ITEM_ID = this.formInline2.itemCode
      this.temp.CHECK_ITEM_NAME = this.formInline2.itemName
    ...
    bindEquAndItem(id, data) {
      CheckPlan.UpdateCheckPlan(id, data)
        .then((response) => {
          if (response.Code === 200) {
    ...
    handleNodeClick(data) {
      ...
      if (data.children != null) {
        this.getCheckItemsByTypeId(this.listQuery2)
      }
    },
    // 节点点击接口函数
    getCheckItemsByTypeId(data){
      CheckItem.GetItemsWithTypeId(data).then((response)=>{
        if(response.Code===200){
    ...
    handleSelectionChange2(val) {
      ...
      this.formInline2.itemCode = this.multipleSelection2[0].CHECK_ITEM_ID
      this.formInline2.itemName = this.multipleSelection2[0].CHECK_ITEM_NAME
    },
    // 设备和点检项弹窗数据行点击事件
    setCurrent(val) {
      this.$refs.singleTable.clearSelection()
      this.$refs.singleTable.toggleRowSelection(val)
      this.formInline2.itemCode = val.CHECK_ITEM_ID
      this.formInline2.itemName = val.CHECK_ITEM_NAME
    },
    // 获取所有
    getAll() {
      //this.getCheckItem()
    ...
    getEquType() {
      EquInfo.GetAllEquType().then((response) => {
        ...
        this.equTypesTree = listToTreeSelect(temp)
      })
    },
    // 获取树
    async getTree() {
    ...
    // 点检项获取函数
    getCheckItem() {
      CheckItem.QueryAndPage(this.listQuery2).then((response) => {
        if(response.Code===200){
          this.checkItemList = response.Data
          this.total2 = response.Total
        }
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          this.updateCheckPlan(id, this.temp)
        }
      })
    },
    // 更新函数(使用的跟绑定函数是同一个)
    updateCheckPlan(id, data) {
      CheckPlan.UpdateCheckPlan(id, data).then((response) => {
        if (response.Code === 200) {
          this.dialogFormVisible = false
          this.$notify({
            title: '成功',
    ...
    // 单行删除接口函数
    deleteCheckPlan(id) {
      CheckPlan.DeleteCheckPlan(id).then((response) => {
        if (response.Code === 200) {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    //     if (element.EQU_NAME === null || element.CHECK_ITEM_NAME === null) {
    //       this.$notify({
    //         title: '提示信息',
    //         message: '未绑定设备或点检项目，请先绑定再进行点检',
    //         type: 'warning',
    //         duration: 2000
    //       })
    //       return
    ...
    //     const flag = id + '-' + (num++).toString()
    //     this.beginCheck(id, element)
    ...
    CheckBegin() {
      ...
      for (let i = 0; i < this.multipleSelection.length; i++) {
        ...
        if (element.EQU_NAME === null || element.CHECK_ITEM_NAME === null) {
          this.$notify({
            title: '提示信息',
            message: `${element.PLAN_ID}计划未绑定设备或点检项目，请先绑定再进行点检`,
            type: 'warning',
            duration: 2000
          })
          continue
